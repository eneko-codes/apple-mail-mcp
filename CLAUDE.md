...
No tool modifies, moves, or deletes an existing message — a mailbox is a record. `send_mail` cannot be undone and needs owner confirmation before every send; a test send is live data — it needs the owner's go-ahead under the rule below, and when allowed must be clearly marked `TESTING: ...` in the subject and go only to the owner's own address.
